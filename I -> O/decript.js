...
function decrypt(word) {
    let result = ""
    let prevLetterCode = 1
    for (let i = 0; i < word.length; i++) {
        let newletterCode = word[i].charCodeAt()
        newletterCode = newletterCode - prevLetterCode
        while (newletterCode < 97) {
            newletterCode += 26
        }
        result += String.fromCharCode(newletterCode)
        prevLetterCode += newletterCode
    }
    return result

}

console.log(decrypt("dnotq")) // should return "crime"